---
title: "Como se Proteger de Sites Falsos"
date: 2021-05-13 19:47:45 -0300
tags: [compras, fraude, segurança, compras-online, online]
description: "Se você encontrar um site duvidoso na internet aconselho agir de acordo com os passos abaixo. Utilizo de meu conhecimento em software para demonstrar como uma pessoa qualquer…"
canonical_url: https://franzkurt.medium.com/como-se-proteger-de-sites-falsos-7f3196d539f0
medium_url: https://franzkurt.medium.com/como-se-proteger-de-sites-falsos-7f3196d539f0
---
### E pesquisá-los corretamente!

Se você encontrar um site duvidoso na internet aconselho agir de acordo com os passos abaixo. Utilizo de meu conhecimento em software para demonstrar como uma pessoa qualquer pode — e deve, juntar informações relevantes a denúncia e como encaminhá-las corretamente aos responsáveis para uma que uma posterior investigação seja aberta.

Mesmo que o site não apresente indícios de irregularidade acredito que este artigo traga opções interessantes afim de melhorar a confiança nas compras online. Meu objetivo aqui é clarificar apessoas leigas no assunto assim como aqueles que nunca comprar e detém dúvidas sobre os processos envolvidos nas vendas em sites ou marketplaces.

### Passo 1: Identifique o cadastro do site e o dono do registro.

**Motivo:** Encontrar diferenças entre a descrição da empresa no site de vendas e compará-lo com dados de outros locais a fim de verificar a veracidade dos dados apresentados.

Os sites fraudulentos geralmente são registrados com pressa, apresentam datas de criação muito recentes ou dados incompletos. Além disso qualquer pessoa pode registrar um nome de domínio no site brasileiro de registro, basta apresentar um nome, um número de CPF, um e-mail para contato e pagar a taxa de 15 reais aproximadamente. Aqui fica claro que falsários mal intensionados utilizam CNPJ ou CPF públicos e muitas vezes dados de contato duvidosos.

**Obs:** Existem lojas que explicam na descrição do site que estão presentes no mercado há anos mas seu registro é deste mês.

### Como encontrar o dono de um domínio?

Na área de levantamento de informações existem inúmeros utilitários e ferramentas, basta buscar pela sigla abaixo:

> OSINT — Inteligência de Fontes Abertas, em inglês.

Um dos mais importantes serviços no que se refere a informações de sites é o utilitário [***whois***](https://pt.wikipedia.org/wiki/WHOIS) que traz informações importantes como contato, número de documentos e e-mail associado a nomes de domínio. Um possível contato com os donos do site. No Brasil os nomes de domínio terminam em .br e podem ser pesquisados neste [***link***](https://registro.br/tecnologia/ferramentas/whois/)***.***

### Validando as informações:

Como exemplo utilizarei uma empresa idônea chamada Americanas.

Mas os passos descritos aqui funcionam para qualquer outro. Buscamos aqui encontrar incoerência entre a suposta história da empresa e a data de sua criação.

> Domínios com data de criação menores que um ano são MUITO suspeitos.

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/01.png" alt="">
</figure>

**Obs:** O site do brasileiro não aceita que coloquemos http:// antes do nome!

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/02.png" alt="">
</figure>

Com as informações acima é possível localizar a empresa e dados de contato.

***Obs:*** Existirão momentos em que um CPF será exibido e você para comprovar a legitimidade deve realizar uma simples busca no [***google***](https://www.google.com) ou [***duckduckgo***](https://duckduckgo.com/).

Caso o CPF informado apareça publicamente, seja em tabelas públicas ou um servidor do governo que possui informações particulares publicas é possível que seja um golpe.

Nada poderemos afirmar neste caso: o CPF pode ter sido usado pelo dono ou não pois a informação é de conhecimento de todos — até dos falsários.

***Outra Obs:*** Lojas grandes não tem CNPJ como Empresário Individual ou no CPF de alguém.

> Para termos certeza quanto as informações podemos validar o CNPJ no site da Receita Federal neste [***link***](http://www.receita.fazenda.gov.br/PessoaJuridica/CNPJ/cnpjreva/Cnpjreva_Solicitacao.asp);

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/03.png" alt="">
</figure>

Como podemos verificar a empresa está ativa e possui os pre-requisitos para atuar no comércio varejista. Validamos assim o segmento e seu cadastro.

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/04.png" alt="">
</figure>

***Obs:*** E-mails com cadastro no gmail ou outro servidor gratuito tem baixo suporte profissional, em outras palavras, uma empresa que se preocupa em oferecer um bom serviço provavelmente terá um servidor de e-mail próprio.

Na figura acima vemos que a Americanas utiliza um domínio próprio:

> admdominios@b2wdigital.com

### Passo 2: Verifique o selo E-Bit

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/05.jpg" alt="">
</figure>

**Motivo:**

O selo faz parte de uma organização sem fins lucrativos e é responsável por verificar as lojas online, além de fornecer medalhas de bronze, prata e ouro para o site pelo atendimento prestado.

### **Como Conferir?**

O site deverá apresentar a imagem do selo E-bit e ao clicar sobre a imagem o usuário **deverá ser redirecionado** para o site do E-bit no qual a empresa **deverá estar corretamente listada**.

Caso a empresa **não esteja no site** do E-bit ou a **imagem não apresente um link direto** para o site oficial, desconfie.

Site oficial: [https://www.ebit.com.br/](https://www.ebit.com.br/) para pesquisar.

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/06.jpg" alt="">
</figure>

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/07.jpg" alt="">
</figure>

### Passo 3: Verifique reclamações no site público Reclame Aqui

***Motivos:*** O site de denúncias contra empresas simplificou a comunicação das empresas com os clientes e apresenta feedbacks cada vez maiores sobre os atendimentos prestados e assim apresenta uma boa base de dados para evitar fraudes.

**Como utilizar:** O site possui uma ferramenta de busca própria, portanto realize a pesquisa sobre o site e verifique se existem reclamações não respondidas. Isto indica indiferença por parte da empresa. Por algum motivo o Reclame Aqui mostra apenas os dados do último semestre, mas basta pesquisar um site e **clicar em Geral** que todas as reclamações aparecem e uma nota de 0 a 10 referente a prestação de serviço é mostrada. [***Link***](https://www.reclameaqui.com.br/)***;***

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/08.jpg" alt="">
</figure>

### Passo 4: Caso existam dúvidas faça uma cópia do site

Para evitar que uma investigação posterior seja rejeitada por inatividade do site é possível realizar um screenshot, uma cópia exata naquele instante.

O site clássico para isso é o [***archive.com***](http://web.archive.org/save/) ou maquina do tempo!

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/09.png" alt="">
</figure>

### Passo 5: Verifique qual é o meio de pagamento — importante!

Normalmente sites caseiros ou com layout amador não tem condições de atender a jurisdição que dita as regras para compras online portanto não aceite pagamentos como transferência bancária direta ou tranferências PIX — que não possuem meios de retornar o pagamento caso seja constatada alguma irregularidade.

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/10.png" alt="">
</figure>

Geralmente sites confiáveis utilizam gateways testados por especialistas de segurança sendo os mais conhecidos o Mercado Pago e a Getnet, entre outros.

Porém há um equívoco em acreditar que por causa da auditabilidade e compliance das empresas elas são confiáveis enquanto que o usuário da mesma é que estará recebendo o valor e pode ter informado dados falsos quando cadastrou seu perfil em um destes gateways de pagamento digital.

Por isso é importante guardar o máximo de informações possíveis como o CPF do envolvido, o ID ou registro no gateway — número que deve estar visível na hora de gerar o boleto de pagamento ou mesmo no site.

### Passo 6: Validando um email qualquer

O site hunter.io permite que através de um simples cadastro sejam realizas pesquisas detalhadas sobre o e-mail. O mais importante seria a data de criação do mesmo e e-mails similares que poderão servir de prova relacional.

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/11.jpeg" alt="">
</figure>

### Passo 7: Buscando posts e colaboradores da empresa

Para ludibriar compradores é comum que as empresas se descrevam como tendo anos no mercado mas uma breve consulta nas redes sociais não apresentam posts ou colaboradores? Algo de errado não está certo!

Com o e-mail adquirido através do whois é possível utilizar uma ferramenta chamada Sherlock que busca centenas de redes pelo usuário.

[***Link***](https://github.com/sherlock-project/sherlock) do projeto que necessita ser instalado.

<figure>
  <img src="/assets/img/medium/como-se-proteger-de-sites-falsos/12.png" alt="">
</figure>

### Dicas Extras

Coloque no cadastro das lojas uma sigla que represente aquela loja especificamente. Eu, o autor deste artigo me chamo franz, assim quando me cadastro em uma loja como a americanas (em um formulário de site) eu coloco o nome assim (aproximadamente): “*franz am*”

Assim eu saberei se uma propaganda chegar em meu e-mail com o seguinte conteúdo:

> *Boa tarde* ***franz am***

> Temos uma novidade!

Eu saberei a origem e de onde retiraram meus dados de contato. Obviamente aqui utilizei a mesma sigla da americanas mas poderia ter utilizado qualquer nome. Poderia criar uma tabela com sobrenomes e cada um significar um cadastro — aqui vale a criatividade do usuário.

Boas compras!
